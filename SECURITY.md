# Jarvis — Politique de Sécurité

Jarvis est conçu comme un assistant personnel **sans restrictions** pour un usage privé. La sécurité est configurée pour un opérateur unique de confiance.

> **Mode personnel** : Par défaut, Jarvis fonctionne en mode déverrouillé — pas de sandbox, pas de pairing DM, accès complet à tous les outils. C'est le comportement attendu pour un assistant personnel.

Si vous souhaitez signaler un problème de sécurité dans le code sous-jacent (OpenClaw), veuillez le rapporter en privé.
...
## Modèle de Confiance Personnel (Jarvis)

Le modèle de sécurité de Jarvis est « assistant personnel » (un opérateur de confiance, potentiellement plusieurs agents).

- Jarvis a accès complet au système, aux fichiers, au réseau, et à la domotique.
- Tous les outils sont activés par défaut : browser, canvas, nodes, cron, exec.
- Pas de sandbox par défaut — Jarvis tourne avec les permissions de l'utilisateur.
- Pas de DM pairing — Jarvis répond directement sur tous les canaux configurés.
