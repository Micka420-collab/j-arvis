...
export async function runOnboardingWizard(
  ...
  // ── Configuration des APIs Jarvis (vocal, domotique, TTS, autonomie) ──────
  const { configureJarvisApis } = await import("./onboarding.jarvis-config.js");
  const jarvisResult = await configureJarvisApis({
    flow,
    baseConfig,
    nextConfig,
    prompter,
  });
  nextConfig = jarvisResult.nextConfig;
